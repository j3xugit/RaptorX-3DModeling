...
def ValidateAllData(validData, validate, modelSpecs, forRefState=False):
	...
	errs = []
		...
		#input = TrainUtils.FetchOneBatch(sharedQ, modelSpecs, forTrain=False)
		input = TrainUtils.PrepareInput4Validate(batch, modelSpecs)
...
def RunOneEpoch(epoch, data, chkpoint, params, other_params, train, quickValidate, fullValidate, modelSpecs):
	...
	## validData are a list of proteins with real data, which are is a Python dict() with keys: name, seqLen and folders for input and labels
	## trainData is a list of locations for training proteins. We have to load the real data for training proteins
	...
	#trainSeqData = DataProcessor.SplitData2Batches(trainDataLocation, numDataPoints=modelSpecs['minibatchSize'], modelSpecs=modelSpecs)
        ...
        #random.shuffle(trainSeqData), here we just use trainSeqData to approximately estimate the number of batches. the real train data is loaded by another process
	n_train_batches = len(trainSeqData)

	#sharedQ = multiprocessing.Queue(config.QSize(modelSpecs) )
	#dataLoader = multiprocessing.Process(target=TrainUtils.DataLoader, args=(sharedQ, trainSeqData, modelSpecs,))
	#dataLoader.setDaemon(True)
	#print 'Start the loader for training data...'
	#dataLoader.start()
	...
	chkpt_save_frequency = max(1, min(validation_frequency, np.int32( len(trainSeqData) / 30 ) ) )
		...
		global trainSharedQ
		input = TrainUtils.FetchOneBatch(trainSharedQ, modelSpecs)
		train_loss, train_errors, param_L2 = train(*input)
		...
		## validate the model	
			...
			global validData, validSharedQ, validDataLoader
	...
	## terminate dataLoader when one epoch is done
	##print 'Stop the loader for training data...'
...
def TrainModel(modelSpecs, trainValidData=None, predDataFile=None):
	...
	del validData[:]
		...
		for batch in predBatches:
			d = DataProcessor.LoadRealData(batch, modelSpecs)
			predData.append(d)
...
def main(argv):
	...
	trainSharedQ = multiprocessing.Queue(config.QSize(modelSpecs) )
        trainDataLoader = multiprocessing.Process(target=TrainUtils.TrainDataLoader, args=(trainSharedQ, trainMetaData, modelSpecs,))
        print 'start the train data loader...'
	...
	maxLen = 800
	validDataLocation, numExcluded = DataProcessor.FilterByLength(validDataLocation, maxLen)
	print 'Exclude ', numExcluded, ' validation proteins longer than ', maxLen, ' AAs'
	...
	## exclude those large proteins from validation since they may not fit into multiprocessing.Queue

	global validSharedQ, validDataLoader
	validSharedQ = multiprocessing.Queue(len(validSeqData) )
	#sharedQ = queue.Queue(len(validSeqData) )
        #dataLoader = threading.Thread(target=TrainUtils.DataLoader, args=(sharedQ, validSeqData, modelSpecs, False))
        validDataLoader = multiprocessing.Process(target=TrainUtils.DataLoader, args=(validSharedQ, validSeqData, modelSpecs, False))
	...
	for batch in validSeqData:
		batches4valid.append(sharedQ.get() )
	dataLoader.join()
	"""

	"""
	print 'Loading validation data...'
	validData = [ DataProcessor.LoadRealData(batch, modelSpecs) for batch in validSeqData ]
	"""
	...
	print 'time spent before training :', beforeTrainTime - startTime
        ...
        #result = TrainModel(modelSpecs=modelSpecs, trainMetaData=trainMetaData, validData=batches4valid)
        result = TrainModel(modelSpecs=modelSpecs, trainValidData=(trainSeqData, validSeqData))

	print 'stop the train data loader...'
	trainDataLoader.terminate()
	trainDataLoader.join()
