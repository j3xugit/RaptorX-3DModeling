...
from utils import SampleBoundingBox, Compatible, str_display, ToFloatX, SplitList
...
def RunOneEpoch(epoch, data, chkpoint, params, other_params, train, quickValidate, fullValidate, modelSpecs):
	...
	chkpt_save_frequency = max(1, min(validation_frequency, np.int32( len(trainSeqData) / 30 ) ) )
		...
		global trainSharedQ
		input = ToFloatX( trainSharedQ.get() )
		train_loss, train_errors, param_L2 = train(*input)
...
def TrainModel(modelSpecs, trainValidData=None, predDataFile=None):
	...
	del validData[:]
		...
		for batch in predBatches:
			data = DataProcessor.LoadRealData(batch, modelSpecs)
...
def main(argv):
	...
	for i, metaData in zip(xrange(numTrainDataLoaders), metaDatas):
        	trainDataLoader = multiprocessing.Process(name='TrainDataLoader ' + str(i) + ' for ' + str(os.getpid()), target=TrainUtils.TrainDataLoader, args=(trainSharedQ, metaData, modelSpecs,))
	...
	## some large validation proteins are excluded since they may not fit into multiprocessing.Queue
	...
	validSharedQ = multiprocessing.Queue(len(validSeqData) )
        validDataLoader = multiprocessing.Process(name='ValidDataLoader for '+str(os.getpid()), target=TrainUtils.ValidDataLoader, args=(validSharedQ, validSeqData, modelSpecs))
	...
	print 'stop the train data loaders...'
	for trainDataLoader in trainDataLoaders:
