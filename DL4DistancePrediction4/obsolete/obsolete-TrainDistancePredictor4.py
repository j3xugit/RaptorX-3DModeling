...
def ValidateAllData(batches, validate, modelSpecs, forRefState=False):
	...
	sharedQ = multiprocessing.Queue(config.QSize(modelSpecs) )
        dataLoader = multiprocessing.Process(target=TrainUtils.DataLoader, args=(sharedQ, batches, modelSpecs, False))
        #print 'Start the data loader for validation...'
        dataLoader.start()
	...
	#print 'Stop the data loader for validation...'
...
def RunOneEpoch(epoch, data, chkpoint, params, other_params, train, quickValidate, fullValidate, modelSpecs):
	...
	print 'Start the data loader for training...'
	dataLoader.start()
	...
	print 'Stop the data loader for training...'
	dataLoader.terminate()
