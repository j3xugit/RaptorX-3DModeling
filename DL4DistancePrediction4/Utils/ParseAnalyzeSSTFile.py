...
if len(sys.argv) < 4:
	print 'Usage: ParseSSTFile.py sstFile tplFile distcbFile'
	exit(-1)
...
assert (distcb.shape[0] == fullSeqLen)
...
for c in content[7: 7+seqLen]: 
	...
	if p1 > 0:
		...
		if InvalidValue( distcb[realIndex, DSSP2SEQRES[p1-1] ] ):
			print '-1 for the distcb between residue and p1 in sst file:', resIndex, p1, realIndex, DSSP2SEQRES[p1-1]
	if p2 > 0:
		BetaPairings[realIndex, DSSP2SEQRES[p2-1] ] = 1
		if InvalidValue( distcb[realIndex, DSSP2SEQRES[p2-1] ] ):
			print '-1 for the distcb between residue and p2 in sst file:', resIndex, p2, realIndex, DSSP2SEQRES[p2-1]
	...
	if donorTo1 > 0:
		...
		if InvalidValue( distcb[realIndex, DSSP2SEQRES[donorTo1-1] ] ):
			print '-1 for the distcb between residue and donorTo1 in sst file:', resIndex, donorTo1, realIndex, DSSP2SEQRES[donorTo1-1]
	if donorTo2 > 0:
		HB[realIndex, DSSP2SEQRES[donorTo2-1] ] =1
		if InvalidValue( distcb[realIndex, DSSP2SEQRES[donorTo2-1] ] ):
			print '-1 for the distcb between residue and donorTo2 in sst file:', resIndex, donorTo2, realIndex, DSSP2SEQRES[donorTo2-1]
...
result = dict()
result['name'] = name
result['notes'] = 'this dictionary contains the parsing result of .sst file generated by PROMOTIF v3'
