...
import CCMpredUtils2 as CCMpredUtils
import MSAUtils

def LoadFeaturePKL(name, location='Feature4Train_2017_E001_PKL/', modelSpecs=None, bounds=None):
	...
	if bUseCCMsum:
		if not extra.has_key('sumCCM'):
			...
			exit(1)
		feature['sumCCM'] = CCMpredUtils.ExpandMatrix(extra['sumCCM'], seqLen, bounds=bounds)

	if bUseCCMraw:
		if not extra.has_key('rawCCM'):
			print 'ERROR: CCM raw matrix is requested, but the file does not have it: ', extrafile
			exit(1)
		feature['rawCCM'] = CCMpredUtils.ExpandMatrix(extra['rawCCM'], seqLen, bounds=bounds)
