...
class LogRegLayer(object):
    ...
    def NLL(self, y, sampleWeight=None):
        ...
        if sampleWeight is not None:
            return -T.sum(T.mul(sampleWeight, T.log(self.p_y_given_x)[T.arange(y.shape[0]), y] ) )/(0.000001 + T.sum(sampleWeight))
        else:
...
class NN4LogReg(object):
	...
	assert (sampleWeight.ndim == 2)
	return T.sum( T.mul(err, sampleWeight), axis=0)/(0.000001 + T.sum(sampleWeight))
