...
class ResConv2DLayer:
    	def __init__(self, rng, input, n_in=0, n_out=0, halfWinSize=0, mask=None, activation=T.nnet.relu):
...
## ResBlockV21 is almost same as the below ResBlockV2, but ResBlockV21 removes unused BatchNormLayer
class ResBlockV21:
...
##ResBlockV22 is different from ResBlockV21 and ResBlockV2 in that ResBlockV22 now has two BatchNormLayers in a ResBlock
## ResBlockV22 may not work as well as ResBlockV2 and ResBlockV21
## ResBlockV2 is default so that our deep network is compatible to previously-trained models. Otherwise we shall use ResBlockV21 ?
...
class ResNet:
    def __init__(self, rng, input, n_in, halfWinSize=0, mask=None, n_hiddens=None, n_repeats=None, activation=T.nnet.relu, dim_inc_method='partial_projection', batchNorm=True, version='SeqResNet'):
	"""
	...
	ResBlock = ResBlockV2
	if version.endswith('V21'):
		ResBlock = ResBlockV21
