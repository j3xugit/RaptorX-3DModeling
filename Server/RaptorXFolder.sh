...
function Usage()
{
	echo $0 "[ -o outDir | -g gpu | -m MSAmethod | -n numDecoys | -r runningMode | -R remoteAccountInfo | -t machineType ] inputFile"
	echo "	This script predicts angle/contact/distance/orientation of a protein and then folds it if needed"
	...
	echo "		Before running this script, some externals software packages shall be installed in your own account. See README.md for details"
	...
	echo "		when the protein large, it may need GPUs with more than 12G memory"
	echo "	-m: an integer indicating MSA generation methods defined by combining 1, 2, 4, 8 and 16, default $MSAmethod"
        echo "	    	1: run HHblits to generate MSA for threading only"
	echo "		2: run HHblits 2.0 to generate MSA for contact and distance prediction (obsolete)"
        echo "	    	4: run Jackhmmer to generate MSA for contact and distance prediction (slow)"
	echo "		8: run HHblits 3.0 to generate MSA for contact and distance prediction (fast)"
        echo "	    	16: search MetaGenome database for each MSA generated by above methods (slow)"
        echo "	    	By default, building MSAs for threading and contact/distance using HHblits without metagenome data"
	echo " "
	...
	echo "		The relaxation step is the most time-consuming. When the protein is large (>600AAs), it may take a few hours to relax one initial 3D model on a single CPU."
	echo "	-R: run the folding module at a remote account specified by this option, e.g., raptorx@raptorx3.uchicago.edu:Work4Server/"
	echo "		Please make sure that you may scp and ssh to this remote account without password. By default, folding is done locally"
...
if [ $seqLen -gt 1050 ]; then
	echo "WARNING: the protein sequence in $seqFile is too long and no 3D models will be built"
	exit 0
fi
