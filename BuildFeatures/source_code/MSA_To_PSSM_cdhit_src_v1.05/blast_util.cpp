 ...
 */
double s_LogDerivative(int order, double* u)
{
  int    i;
 ...
 */
double s_GeneralLnGamma(double x, int order)
{
   int       i;
   double     xx, tx;
   ...
   double    tmp, value;
   double    *coef;
   int      xgamma_dim = DIM(_default_gamma_coef);
 ...
 */
double s_PolyGamma(double x, int order)
{
   int      k;
   double   value, tmp;
...
//--------- other BLAST math functions ------------//
double BLAST_Factorial(int n)
{
   if (n < 0)
      return 0.0; /* Undefined! */

   if (n < (int)DIM(kPrecomputedFactorial))
...
{
   if ( (n > 1) && (n < (int)DIM(kPrecomputedFactorial) ) ) {
...
{
   double   romb[20];   /* present list of Romberg values */
   double   h;   /* mesh-size */
   int      i, j, k, npts;
...
void BLAST_FreqNormalize (double *input,int size,double factor)
{
	int i;
	double sum=0.0;
	for(i=0;i<size;i++)sum+=input[i];
	if(sum<=0.0)return;
	for(i=0;i<size;i++)input[i]=factor*input[i]/sum;
}
