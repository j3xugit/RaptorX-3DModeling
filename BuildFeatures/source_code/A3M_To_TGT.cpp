...
// command to generate features 
 	string DistFeatureHome = getenv("DistFeatureHome");
	if (DistFeatureHome.empty())
		cerr<<"Please set the environmental variable DistFeatureHome"<<endl;

	string genMAIN_cmd = DistFeatureHome + "/util/genMAIN.sh " + input_a3m + " "+temporary_root+"/";
	string genSS3_cmd = DistFeatureHome + "/util/genSS3.sh " + mtxfile1 + " "+temporary_root+"/";
	string genDIS_cmd = DistFeatureHome + "/util/genDIS.sh " + mtxfile1 + " "+temporary_root+"/";
	string genSS8_cmd = DistFeatureHome + "/util/genSS8.sh " + targetName + " "+temporary_root+"/";
	string genACC_cmd = DistFeatureHome + "/util/genACC.sh " + targetName + " "+temporary_root+"/";
// generate MAIN file
	{
		cout << genMAIN_cmd <<endl;
		system(genMAIN_cmd.c_str());
	}
// generate SS3 file
	{
		cout << genSS3_cmd <<endl;
		system(genSS3_cmd.c_str());
	}
// generate DIS file
	{
		cout << genDIS_cmd <<endl;
		system(genDIS_cmd.c_str());
	}
// generate SS8 file
	{
		cout << genSS8_cmd <<endl;
		system(genSS8_cmd.c_str());
	}
// generate ACC file
	{
		cout << genACC_cmd <<endl;
...
// structural info
	...
	fout.close();	
	string fin_combine="";
	fin_combine = fin_combine + " " + DistFeatureHome + "/util/combine_file.sh "+temporary_root+"/"+targetName+".ss2 "+temporary_root+"/"+targetName+".ss8 "+temporary_root+"/"+targetName+".acc "+output_tgt;
...
// ---------- build ss3 and acc using SS8_To_SS3 and ACC_ReFormat ------------- //__2014_10_30__//
	string nnn_command;
	nnn_command="";
	nnn_command = nnn_command + " " + DistFeatureHome + "/util/SS8_To_SS3 "+temporary_root+"/"+targetName+".ss8 > "+temporary_root+"/"+targetName+".ss3";
	system(nnn_command.c_str());
	nnn_command="";
	nnn_command = nnn_command + " " + DistFeatureHome + "/util/ACC_ReFormat "+temporary_root+"/"+targetName+".acc "+temporary_root+"/"+targetName+".seq > "+temporary_root+"/"+targetName+".acc_";
	system(nnn_command.c_str());
	nnn_command="";
