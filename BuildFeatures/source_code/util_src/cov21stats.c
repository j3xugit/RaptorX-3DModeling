...
int             main(int argc, char **argv)
{
    ...
    double sum, score, di, pdir, wtsum, idthresh=0.38, (** pab)[21][21], pa[MAXSEQLEN][21];
    float *weight, val;
    ...
    if (argc != 3 && argc != 4)
	fail("Usage: cov21stats [-p] alnfile outputfile");
	...
	for (b=0; b<21; b++)
		...
		for (j=0; j<seqlen; j++)
		{
		    val = pab[i][j][a][b] - (pairflg ? 0.0 : pa[i][a] * pa[j][b]);
