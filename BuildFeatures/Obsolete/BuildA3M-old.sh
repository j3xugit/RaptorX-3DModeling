...
DB4Thread=$HHPkg/databases/uniclust30_2017_10/uniclust30
DB4HHblits=$HHPkg/databases/uniclust30/uniclust30

HHS=$HHPkg/hhsuite3
if [ ! -d $HHS ]; then
        echo "ERROR: invalid hhsuite v3.0 folder $HHS "
        exit 1
fi

JackPkg=$DistFeatureHome/EVAlign/
DB4Jack=${JackPkg}/databases/uniref90.fasta

MetaDataSearch=$DistFeatureHome/MetaDataSearch/BuildMSAFromMetaData.sh
if [ ! -f $MetaDataSearch ]; then
        echo "ERROR: invalid script for meta data search: $MetaDataSearch"
        exit 1
fi
...
function Usage()
{
	echo $0 "[ -d ResultDir | -m MSAmethod | -t SeqDB4Threading | -h SeqDB4HHblits | -j SeqDB4Jackhmmer | -c numCPUs ] seqFile"
	...
	echo "	-d: the folder for result saving, default current work directory; results will be saved to ResultDir/proteinName_OUT/"
	echo "	-m: an integer indicating MSA generation methods, formed by combining 1, 2, 4, 8 and 16, default $MSAmode"
	echo "	   1: run HHblits to generate MSA for threading only; 2: run HHblits 2.0 to generate MSA for contact "
	echo "	   4: run Jackhmmer to generate MSA for contact; 8: run HHblits 3.0 to generate MSA for contact"
	echo "	   16: search MetaGenome database for each MSA generated by above methods"
	echo "	   By default, building MSAs for threading and contact/distance using HHblits 3.2 and Jackhmmer and metagenome data"
	echo " "
	echo "	-t: sequence database used by HHblits for sequence profile generation, default $DB4Thread"
	echo "	-h: sequence database used by HHblits for MSA generation for contact/distance prediction, default $DB4HHblits"
...
if [ $MSAmode -lt 1 -o $MSAmode -gt 31 ]; then
	echo "ERROR: MSA_mode shall be an integer between 1 and 31"
	exit 1
fi
...
if (( $RunThread )); then
	...
	if [ ! -f ${DB4Thread}_hhm.ffindex ]; then
        	echo "ERROR: damaged sequence database $DB4Thread for threading"
        	exit 1
	fi
	...
	if [ ! -f "$outdir/${relnam}_thread/$relnam.a3m" ] || [ ! -f "$outdir/${relnam}_thread/$relnam.tgt" ]; then
		...
		$HHS/bin/hhblits -i $outdir/$relnam.seq -cpu $numCPUs -d $DB4Thread -o $relnam.hhr -oa3m $relnam.a3m -n $iteration -e $e_value -neffmax $neffmax

		tmp_root=$relnam"_tmp"/
		mkdir -p $tmp_root
		$DistFeatureHome/util/A3M_To_TGT -i $outdir/$relnam.seq -I $relnam.a3m -o $relnam.tgt -t $tmp_root
		#mv $relnam.hhr $relnam.a3m $relnam.tgt $outdir/${relnam}_thread
		rm -rf $tmp_root
	fi
	cd $currDir
fi
...
if (( $RunHHblitsV2 )); then
	if [ ! -f ${DB4HHblits}_hhm.ffindex ]; then
        	echo "ERROR: damaged sequence database $DB4HHblits for HHblitsV2"
        	exit 1
	fi
	...
	if [ ! -f "$MSADir/${relnam}_v2uce3/$relnam.a3m" ] || [ ! -f "$MSADir/${relnam}_v2uce5/$relnam.a3m" ]; then
		echo "Building MSA from $DB4HHblits by hhsuite 2.0 ..."
		iteration=3

		e_value=0.001
		$HHPkg/BuildMSAByHH.sh -i $outdir/$relnam.seq -n $iteration -e $e_value -C -1 -o $MSADir/${relnam}_v2uce3 -d $DB4HHblits -c $numCPUs & 

		e_value=1
		$HHPkg/BuildMSAByHH.sh -i $outdir/$relnam.seq -n $iteration -e $e_value -C -1 -o $MSADir/${relnam}_v2uce5 -d $DB4HHblits -c $numCPUs &
...
#-> 3. generate MSA for contact and distance prediction by HHsuites 3.0
if (( $RunHHblitsV3 )); then
	if [ ! -f ${DB4HHblits}_hhm.ffindex ]; then
        	echo "ERROR: damaged sequence database $DB4HHblits for HHblitsV3"
        	exit 1
	fi
	...
	if [ ! -f "$MSADir/${relnam}_uce3/$relnam.a3m" ] || [ ! -f "$MSADir/${relnam}_uce5/$relnam.a3m" ]; then
		echo "building MSA from $DB by hhsuite 3.0..."
		iteration=3
		#hhsuite=$HHPkg/hhsuite3

		e_value=0.001
		$HHPkg/BuildMSAByHH.sh -i $outdir/$relnam.seq -n $iteration -e $e_value -C -1 -o $MSADir/${relnam}_uce3 -d $DB4HHblits -c $numCPUs -h $HHS &

		e_value=0.00001
		$HHPkg/BuildMSAByHH.sh -i $outdir/$relnam.seq -n $iteration -e $e_value -C -1 -o $MSADir/${relnam}_uce5 -d $DB4HHblits -c $numCPUs -h $HHS &
...
if (( $RunJackhmmer )); then
	if [ ! -f $DB4Jack ]; then
		echo "ERROR: invalid sequence database for Jackhmmer: $DB4Jack"
		exit 1
	fi
	...
	if [ ! -f "$MSADir/${relnam}_ure3/$relnam.a3m" ] || [ ! -f "$MSADir/${relnam}_ure5/$relnam.a3m" ]; then
		echo "building MSA from $DB4Jack by Jackhmmer..."
		iteration=3
...
## remove those folders with empty a3m files
...
if (( $UseMetaData )); then
	...
	do
		...
		$MetaDataSearch -i $outdir/$relnam.seq -I $MSADir/${relnam}_${method}/$relnam.a3m -m 1 -o $MSADir/${relnam}_${method}_meta &	
		sleep 1
	done
	wait
