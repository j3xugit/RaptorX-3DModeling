...
$JackHome/util/Gen_EVfold_config3 $tmp_root/$relnam $seq_file $cpu_num $bitscore $threshold $iteration $uniref90 $JackHome > $tmp_root/$relnam.config_file 

echo "Running evcouplings to generate MSAs with parameter cpu $cpu_num, bitsco $bitscore, thres $threshold, iter $iteration, uniref $uniref90"

## switch to the EVcoupling environment
...
evcouplings -s $seq_file $tmp_root/$relnam.config_file
echo "evcouplings done"

## return to previous env
conda deactivate
eval "$(conda shell.bash hook)"
source activate $currEnv
