...
def Fold(pos, constraints=None, ncycles=1000, tolerance=0.0001, UseNBList=True, UsePerturbation=False):
...
def RelaxWithoutConstraints(pose, ncycles=3):
        assert pose is not None
...
#def Relax(pose=None, cstFile=None, w4distance=0.2, w4angle=0.2, w4dihedral=0.2, w4beta=None, ncycles=5):
def Relax(pose=None, constraints=None, w4distance=0.2, w4angle=0.2, w4dihedral=0.2, w4beta=None, ncycles=5):
        assert pose is not None
	assert constraints is not None
...
def main(argv):
	...
	if not os.path.isfile(cstFile):
		print 'ERROR: invalid constraint file for folding/relax:', cstFile
		exit(1)


	try:
                opts, args = getopt.getopt(argv[2:],"w:d:a:e:n:t:s:rbqp",[ "w4distance=", "w4dihedral=", "w4angle=", "w4beta=", "ncycles=", "tolerance=", "savefolder=", "randomPhiPsi=", "nonblist=", "quick=", "perturbation="])
                #print opts, args
	...
	pose = Fold(pose, constraints, tolerance=tolerance, ncycles=ncycles, UseNBList=UseNBList, UsePerturbation=UsePerturbation)
	if pose is None:
		print 'ERROR: the folded pose is None'
		exit(1)

	## some compute nodes of a cluster have a short process ID. To avoid conflit, we add a random number after the process ID
	...
	if DoRelax:
		...
		pose = Relax(pose, constraints, w4angle=w4angle, w4dihedral=w4dihedral, w4distance=w4distance, w4beta=w4beta)
