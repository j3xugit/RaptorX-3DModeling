...
do
	sbatch -p $QUEUE -J ${target}.FragFold -o ${target}.fragFold.out -e ${target}.fragFold.out -C avx $program $seqfile $FragDir $predMatrixFile $predPropertyFile $FlagTemplateFile $savefolder $numModelsPerJob 
