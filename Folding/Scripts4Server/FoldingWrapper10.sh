...
if [ ! -f ${angleFile} ]; then
	echo "WARNING: file does not exist: " ${angleFile}
	#exit 1

	angleFile=${AngleDir}/${target}.ss3
	if [ ! -f ${angleFile} ]; then
		echo "ERROR: please provide either a predictedProperties.pkl file or a .ss3 file for predicted secondary structure information"
		exit 1
	fi
fi
...
cmdStr="${program} -i $seqFile -d ${distFile} -p ${angleFile} -e ${dihedralEnergyConstant} -t $boundType -a ${atomPairTypes} -c $cutoff -o ${OutDir} -m 20:10"
echo $cmdStr
$cmdStr
